...
import com.example.creditwahana.Activity.Leader.LeaderReadActivity;
...
public class AdapterDataLeader extends RecyclerView.Adapter<AdapterDataLeader.HolderDataLeader>{
    private Context ctx;
    private List<DataModel> listdataleader;

    public AdapterDataLeader(Context ctx, List<DataModel> listdataleader) {
        this.ctx = ctx;
        this.listdataleader = listdataleader;
    }

    @NonNull
    @Override
    public HolderDataLeader onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View layoutleader = LayoutInflater.from(parent.getContext()).inflate(R.layout.card_item, parent, false);
        HolderDataLeader holderleader = new HolderDataLeader(layoutleader);
        return holderleader;
    }

    @Override
    public void onBindViewHolder(@NonNull HolderDataLeader holder, int position) {
        DataModel dm = listdataleader.get(position);
        ...
        holder.kreditleader.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Intent readleader = new Intent(ctx, LeaderReadActivity.class);
                readleader.putExtra(LeaderReadActivity.DATA_DETAIL_LEADER, dm);
                ctx.startActivity(readleader);
            }
        });
    }
    ...
    public int getItemCount() {
        return listdataleader.size();
    }

    public class HolderDataLeader extends RecyclerView.ViewHolder{
        ...
        public HolderDataLeader(@NonNull View itemView) {
            super(itemView);
